...
\begin{itemize}
  ...
  \item \textbf{A published result of ours that turned out to be our own
        defect.} We first reported cross-document entity resolution as a loss for
        the graph. A name-normalisation convention was undocumented in both
        schema descriptions; documenting it identically and changing nothing else
        moved three of four categories (\Cref{sec:harness},
        \Cref{tab:beforeafter}). We show both measurements, because a repair that
        moves a headline in the author's favour should be visible.
  \item \textbf{A cost accounting.} GraphRAG's accuracy is bought:
        ...
        single attempt.
  \item \textbf{A safety result that runs against the obvious intuition.}
        Giving text-to-SQL a better schema description made it more dangerous,
        not less: its confident falsehoods rose from 2 to 6, because knowing the
        schema converted refusals into assertive multi-hop answers that conflate
        people who share a surname. GraphRAG made no false statement in either
        measurement (\Cref{sec:failures}).
\end{itemize}
