...
\textbf{T4 measures an implementation, not an architecture --- and that cuts both
ways.} GraphRAG's loss on T4 is attributable to 0.12\% \texttt{:RESOLVES\_TO}
coverage and a name-normalization mismatch, both fixable. A charitable reading
says the architecture would win T4 once repaired. We resist it: we did not repair
and re-measure, so the charitable reading is untested. The same logic applies in
reverse to the wins --- some part of GraphRAG's T1 and T3 margins may reflect
implementation effort we spent on the graph and not on the baselines.
