...
\title{\vspace{-1.3cm}\textbf{Structural Retrieval versus Similarity Search:\\[0.2em]
A Measured Comparison on SEC EDGAR Filings}\vspace{0.2em}}

\author{%
  Fakabbir Amin\\[0.2em]
  \small\texttt{fakabbir@gmail.com}
}
...
\vspace{0.2em}
\noindent\small\textbf{Keywords:} retrieval-augmented generation, knowledge
graphs, text-to-SQL, entity resolution, provenance, SEC EDGAR, empirical
evaluation.\normalsize
\vspace{0.5em}
...
\centering
\input{gen/fig_subdist.tex}
\caption{EX-21 disclosure per filer.}
\label{fig:subdist}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\input{gen/fig_months.tex}
...
\begin{figure}[htbp]
\centering
\resizebox{\linewidth}{!}{\input{gen/fig_arch.tex}}
...
\begin{subfigure}[t]{0.48\linewidth}
\centering
\input{gen/fig_bytype.tex}
\caption{Attempts passed per category.}
\label{fig:bytype}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\input{gen/fig_outcomes.tex}
...
\begin{subfigure}[t]{0.48\linewidth}
\centering
\input{gen/fig_latency.tex}
\caption{Mean latency per category (log).}
\label{fig:latency}
\end{subfigure}\hfill
\begin{subfigure}[t]{0.48\linewidth}
\centering
\input{gen/fig_cost.tex}
\caption{Tokens against attempts passed.}
\label{fig:cost}
\end{subfigure}
\caption{The economics. (a) Text-to-SQL's T3 mean of 92\,s, worst attempt 445\,s, is the SQL and not the model: the generated queries join three tables without narrowing first. (b) GraphRAG's accuracy is bought, at roughly 17{,}300 tokens per passing attempt against text-to-SQL's 5{,}450.}
\label{fig:econ}
\end{figure}

\begin{table}[htbp]
\centering
\input{gen/tab_repair.tex}
